...
async function startQuery (sdk, { workGroup, resultBucket, resultPrefix, query, token }) {
...
async function runQuery (sdk, s3, {
  ...
  try {
    ...
    const { queryId, outputLocation } = await startQuery(sdk, { workGroup, resultBucket, resultPrefix, query, token, quiet })
    if (!quiet) {
    ...
    try {
      ...
      if (lastBytes !== bytesScanned || lastExecTime !== execDuration) {
        ...
        if (progress === true) {
          ...
          const timeStr = c.blue(millis(execDuration))
