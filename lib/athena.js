...
async function runQuery (sdk, s3, {
  ...
  try {
    ...
    const { queryId, outputLocation } = await startQuery(sdk, { resultBucket, resultPrefix, query, token, quiet })
    if (!quiet) {
    ...
    return { queryId, outputLocation, execDuration, bytesScanned, token, success, timedOut }
  } catch (error) {
    if (!quiet) {
      console.error(error)
