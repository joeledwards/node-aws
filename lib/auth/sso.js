...
class TemporarySsoCredentials extends aws.Credentials {
  ...
  get expired () {
    ...
    return moment.utc(this.expiresAt).diff(moment.utc()) < 300000
  }
  ...
  async refreshPromised () {
    try {
      ...
      if (this._verbose) {
        this.logger.error(`Failed to resolve credentials:`, error)
      } else {
        this.logger.error(`Failed to resolve credentials: ${error}`)
      }
      throw error
  ...
  async fetchClient () {
    try {
      ...
      if (moment.utc(record.expiresAt).diff(moment.utc()) < 300) {
        this.logger.warn('Client registration record expired.')
        return null
      } else {
