...
class TemporarySsoCredentials extends aws.Credentials {
  ...
  async refreshPromised (isInner) {
    try {
      ...
      if (isInner !== true) {
        this.logger.error(`Failed first attempt at resolving credentials. Trying again...\n${error}`)
  ...
  async persistCredentials (credentials) {
    this.logger.verbose(`Persisting temporary credentials to ${credFile} ...`)
    await promised(h => fs.writeFile(credFile, JSON.stringify(credentials), h))
  }
  ...
  async fetchCredentials () {
    try {
      this.logger.verbose(`Fetching credentials from ${credFile} ...`)
  ...
  async deleteClient () {
    try {
      ...
      await promised(fs.unlink(clientFile, h))
  ...
  async deleteAccess () {
    try {
      ...
      await promised(fs.unlink(accessFile, h))
  ...
  async deleteCredentials () {
    try {
      ...
      await promised(fs.unlink(credFile, h))
