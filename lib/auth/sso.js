...
const credentialsFile = path.resolve(clientDir, 'credentials.json')
...
class TemporarySsoCredentials extends aws.Credentials {
  ...
  async refreshPromised (isInner) {
    try {
      ...
      // TODO: GetCallerIdentity from STS
      //new aws.STS()
    } catch (error) {
      if (isInner !== true) {
  ...
  async persistClient (client) {
    this.logger.verbose(`Persisting client info to ${clientFile} ...`)
  ...
  async persistAccess (access) {
    this.logger.verbose(`Persisting access data to ${accessFile} ...`)
    await promised(h => fs.writeFile(accessFile, JSON.stringify(access), h))
  }

  async persistCredentials (credentials) {
    this.logger.verbose(`Persisting temporary credentials to ${credentialsFile} ...`)
    await promised(h => fs.writeFile(credentialsFile, JSON.stringify(credentials), h))
  }

  async fetchClient () {
    try {
      this.logger.verbose(`Fetching client info from ${clientFile} ...`)
  ...
  async fetchAccess () {
    try {
      this.logger.verbose(`Fetching access data from ${accessFile} ...`)
  ...
  async fetchCredentials () {
    try {
      this.logger.verbose(`Fetching credentials from ${credentialsFile} ...`)
      const raw = await promised(h => fs.readFile(credentialsFile, 'utf-8', h))
      const record = JSON.parse(raw)
  ...
  async deleteClient () {
    try {
      ...
      await promised(h => fs.unlink(clientFile, h))
  ...
  async deleteAccess () {
    try {
      ...
      await promised(h => fs.unlink(accessFile, h))
  ...
  async deleteCredentials () {
    try {
      ...
      await promised(h => fs.unlink(credentialsFile, h))
