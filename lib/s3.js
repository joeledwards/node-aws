...
async function scanLogs (sdk, bucket, prefix, scanner) {
  const keys = await scanKeys(sdk, bucket, prefix)
...
async function* scanMpu (sdk, bucket, options = {}) {
  ...
  while (shouldFetchMore()) {
    const {
      ...
      truncated,
      marker
    } = await nextBatch(bucket, uploadMarker, remaining)
  ...
  async function nextBatch (bucket, uploadMarker, limit) {
    const options = {
      Bucket: bucket
    }

    if (uploadMarker) {
      options.UploadIdMarker = uploadMarker
    }
